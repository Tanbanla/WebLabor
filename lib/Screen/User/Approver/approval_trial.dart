...
class _ApprovalTrialScreenState extends State<ApprovalTrialScreen> {
  ...
  Future<void> _reloadData(
    ...
    try {
      ...
      if (groupId == "Section Manager" || groupId == "Admin") {
        ...
        // Đảm bảo danh sách approver được load trước khi giao diện hiển thị dropdown
  ...
  Widget _buildSearchAndActions() {
    ...
    // Lưu ý: Không tính trước requiresNextApprover ở đây vì dataList có thể chưa load.
    // Sẽ tính bên trong Obx để phản ứng với thay đổi data.
            ...
            onPressed: () async {
              try {
                ...
                for (int i = 0; i < dataToExport.length; i++) {
                  ...
                  setCellValue('AB', item.useRApproverChief ?? '');
                }
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow2(
      ...
      cells: [
        ...
        // Group column (allow wrap for long text)
        DataCell(
